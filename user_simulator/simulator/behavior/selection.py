...
from user_simulator.simulator.behavior.library import (
    _BEHAVIOR_ORDER, _BEHAVIORS, _CTRL_SYSTEM_RENDERED, _DEFAULT_BEHAVIOR,
    _SIM_PROJECTION, _TMPL_CTRL_USER,
)
...
async def _select_behavior_with_controller(
    ...
    llm: LLM, config: AblationConfig | None = None,
) -> dict:
    ...
    try:
        raw = await llm.chat(
            ...
            json_mode=True, call_type="behavior_controller",
        )
